...
# = Define: horizon::conf
#
# With this define you can manage any horizon configuration file
#
# == Parameters
#
# [*template*]
...
#   Example: template => 'site/horizon/my.conf.erb',
#
# [*content*]
...
#   Example: source => 'puppet:///site/horizon/my.conf',
#
# [*ensure*]
...
define horizon::conf (
  ...
  $manage_path    = pickx($path, "${horizon::config_dir_path}/${name}")
  $manage_content = default_content($content, $template)
  $manage_mode    = pickx($mode, $horizon::config_file_mode)
  $manage_owner   = pickx($owner, $horizon::config_file_owner)
  $manage_group   = pickx($group, $horizon::config_file_group)
  $manage_require = pickx($config_file_require, $horizon::config_file_require)
  $manage_notify  = $config_file_notify ? {
    'class_default' => $horizon::manage_config_file_notify,
    default         => $config_file_notify,
  }

  file { "horizon_conf_${name}":
    ensure  => $ensure,
    source  => $source,
